...
RISK_ASSETS  = ["VTI", "QQQ", "SCHF", "EEM", "XLV", "XLF"]  # fallback only now
...
def run_adaptive_v3(start="2004-06-30", verbose=True):
    ...
    for i, d in enumerate(all_dates):
        # DYNAMIC UNIVERSE: instead of a fixed RISK_ASSETS list,
        # ask the ETF scorer what's eligible THIS week. Only the
        # equity/risk sleeve is dynamic -- DEF_ASSETS (bonds/gold)
        # stays fixed, same as v3, so this test isolates ONE
        # variable: which equities to hold, not the whole system.
        try:
            week_universe = etf_scorer.select_universe(
                str(d.date()), verbose=False)
            dynamic_risk_assets = [t for t, _ in
                (week_universe["tier1"] + week_universe["tier2"])]
        except Exception:
            dynamic_risk_assets = RISK_ASSETS  # fallback if scorer fails

        if len(dynamic_risk_assets) < 2:
            dynamic_risk_assets = RISK_ASSETS  # fallback if too few candidates

        # Available assets
        avail = [t for t in dynamic_risk_assets + DEF_ASSETS
        ...
        avail_risk = [t for t in dynamic_risk_assets if t in avail]
