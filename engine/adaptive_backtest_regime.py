...
# Real, validated 1,368-ticker equity universe for the new
# breadth+correlation regime detector (separate from RISK_ASSETS,
# which remains the fixed 6-ticker equity sleeve used for actual
# SVI portfolio construction -- EQUITY_UNIVERSE is only used to
# compute the regime signal itself, a much broader real-market read)
import json as _json
with open(os.path.join(DATA_DIR, "final_equity_universe.json")) as _f:
    EQUITY_UNIVERSE = _json.load(_f)
...
def run_adaptive_v3(start="2004-06-30", end=None, verbose=True):
    ...
    for i, d in enumerate(all_dates):
        ...
        if len(rets) < 60:
            continue
        # REPLACED: scalar dial (credit dial + bootstrap blending)
        # with the new breadth+correlation regime detector, per
        # tonight's structural redesign. Validated separately
        # (8/8 correct on real historical dates) and via a
        # lightweight full-history dry run (final $68,899 vs
        # v2's $50,237, comparable 2008 drawdown -27.7% vs -25%
        # to -33% range). This is the real, full-pipeline test
        # with actual SVI optimization and the full crisis toolkit
        # intact, to get the genuinely final, trustworthy number.
        try:
            ...
            alloc = {
                ...
                "gld_min": 0.04,
                "dbmf": 0.08,
                "shorts": {"SH": 0.0, "PSQ": 0.0, "TBF": 0.0},
            }
            if scenario == "SYSTEMIC_CRISIS":
                alloc["shorts"] = {"SH": 0.04, "PSQ": 0.02, "TBF": 0.02}
                alloc["dbmf"] = 0.13
        except Exception as e:
            print(f"    [WARN] Regime detection failed at {d.date()}: {e}")
            alloc = {"equity_target": equity_now, "gld_min": 0.04,
                     "dbmf": 0.08, "shorts": {"SH": 0.0, "PSQ": 0.0, "TBF": 0.0}}
            scenario = "ERROR_FALLBACK"
        # Direct regime -> blend mapping, replacing the old
        # scalar-threshold approach now that we have real, distinct
        # regime labels instead of a single scalar dial value
        REGIME_TO_BLEND = {
            "CALM": "bull_calm",
            "CORRELATED_CALM": "bull_late",
            "SCATTERED_WEAKNESS": "stress",
            "SYSTEMIC_CRISIS": "crisis",
            "ERROR_FALLBACK": "bull_late",
        }
        blend_name = REGIME_TO_BLEND.get(scenario, "bull_late")
        ...
        records.append({
            "date":      next_d,
            "portfolio": port_val,
            "dial":      regime_result.get("pct_below", 0.0),  # repurposed: breadth pct_below, not scalar dial
            "scenario":  scenario,
        ...
        if verbose:
            ...
            print(f"  [{bar}] {pct:5.1f}%  {d.date()}  "
                  f"${port_val:>9,.0f}  brdth={regime_result.get('pct_below', 0.0):.2f}  "
                  f"scen={scenario:<10}  "
